...
//		logger.info("[HttpExecutors--进入HttpExecutors-{},本次任务最大需要重试{}次]", overallStartTime,
//				null == retryMeta ? 0 : retryMeta.getMaxAttempts());
		while (true) {
...
//						/* JSON.toJSONString(result) */" ", exception.getException(),
//						(System.currentTimeMillis() - startTime));
//				logger.info("[HttpExecutors--判断本次HTTP请求是否触发重试]");
				// 判断是否需要重试
				if (null != retryer && retryer.continueOrEnd(retryer, exception, retryMeta)) {
//					logger.info("[HttpExecutors--重试机制生效，已等待：{},开始第{}次重试]", retryMeta.getInterval(),
//							retryMeta.getAttempt() - 1);
					// 进行重试，此时已经睡了间隔时间
					continue;
				} else {
					// 跳出重试
//					logger.info("[HttpExecutors--HTTP请求{}，准备退出]", null == retryer ? "无重试机制"
//							: null == retryMeta || retryMeta.getMaxAttempts() == 0 ? "无重试次数"
//									: retryMeta.getAttempt() > retryMeta.getMaxAttempts() ? "到达重试临界值" : "不满足重试要求");
					if (null != exception.getResult()) {
						// 跳出重试时任务执行没有异常
//						logger.info("[HttpExecutors--退出--HTTP请求状态正常]-[总耗时：{}]",
//								System.currentTimeMillis() - overallStartTime);
						return result;
					} else {
						// 跳出重试时任务执行发生异常
//						logger.error("[HttpExecutors--退出--HTTP请求状态异常]-[总耗时：{}]",
//								System.currentTimeMillis() - overallStartTime);
	...
	public <T> T executor(Task<T> task, boolean needRetry) throws Exception {
		...
		return executor(task, needRetry ? DEFAULT_RETRYER : null,
				needRetry ? new Retryer.RetryMeta() : Retryer.RetryMeta.NO_RETRY);
	}
